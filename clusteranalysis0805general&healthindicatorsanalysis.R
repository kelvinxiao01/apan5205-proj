...
high_mmr_data <- merged_with_clusters %>%
  filter(Cluster == "High MMR")

#--- 38 - Country-level correlations by period in High MMR cluster
...
#--- 40 - Calculate period differences for all clusters and countries
corr_country_period <- merged_with_clusters %>%
  group_by(country_name, Cluster, period) %>%
  summarise(
    corr_MMR_HIV_Women = safe_cor(MMR, HIV_Women),
    corr_MMR_HIV_Total = safe_cor(MMR, HIV_Total),
    corr_MMR_AFR = safe_cor(MMR, AFR_Level_num),
...
# Pivot for periods with clean names
...
names(corr_country_wide) <- make.names(names(corr_country_wide))

#--- 41 - Compute differences between periods
corr_country_wide <- corr_country_wide %>%
  mutate(
    ...
    diff_mean      = rowMeans(across(starts_with("diff_")), na.rm = TRUE)
  )

#--- 42 - Identify top beneficiaries and worst performers per cluster
top_bottom_by_cluster <- corr_country_wide %>%
  group_by(Cluster) %>%
  summarise(
    top_beneficiaries = paste(country_name[order(-diff_mean)][1:5], collapse = ", "),
    worst_performers  = paste(country_name[order(diff_mean)][1:5], collapse = ", "),
    .groups = "drop"
  )

#--- 43 - Display table with gt
top_bottom_by_cluster %>% gt()